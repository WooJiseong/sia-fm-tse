...
test_dataset = LibriDataset_single_emb(
    f"data/LibriSpeech/LibriSpeech/_test_data",
    ...
    return_dvec=("tfgridnet" in args["model_name"] and "dvec" in args["fusion_name"]),
    dvec_rate=dvec_rate,
    include_silent=False,
    special_spk=special_spk,
    partial_range=PI_range,
    neg_partial_range=NI_range,
    reverb="all",
    brir_dir=brir_dir,
    binaural=True,
    reverb_cond=reverb_cond,
    noise_dir=noise_dir + "tt/",
    zero_degree_pos=zero_degree_pos,
    same_disturb=False,
)
...
print(
    f"evaling {' '.join([k + ':' + str(args[k]) for k in args])}, zero degree pos: {zero_degree_pos}, special spk: {special_spk}, PI range: {PI_range}, NI range: {NI_range}"
)
print(
    f"{test_dataloader.__class__.__name__} {source_num}C{''.join([str(n) for n in active_num])}, enroll num: {enroll_num}, wave length {wave_length}, pos {pos_example_length}, neg {neg_example_length}"
)
mses, snrs, si_snrs, imp_snrs, imp_si_snrs = [], [], [], [], []
pesqs, stois, dnsmoses, wers = [], [], [], []
for epoch in range(repeat_num):
    mse_i, snr_i, si_snr_i, imp_snr_i, imp_si_snr_i, pesq_i, stoi_i, dnsmos_i = eval(
        model=model, dataloader=test_dataloader, epoch=epoch
    )
    mses.append(mse_i)
    snrs.append(snr_i)
