...
if args["model_name"] == "tfgridnet_causal":
    ...
    model.load_state_dict(
        torch.load(args["model_path"], map_location=torch.device("cpu"))["state_dict"],
        strict=True,
    )
    sample_rate = 16000
...
def eval(model, dataloader, epoch):
    ...
    with torch.no_grad():
        for i in tqdm(range(set_size)):
            ...
            for i in range(partial_num):
                active_len = int(
                    pos_example_length
                    * random.uniform(partial_range[0], partial_range[1])
                )
            ...
            for i in range(enroll_num - 1):
                active_len = int(
                    neg_example_length
                    * random.uniform(neg_partial_range[0], neg_partial_range[1])
                )
            ...
            if args["model_name"] == "tfgridnet_causal":
                ...
                cond_emb, pos_std, dec_audio = model.encoder_pos_neg(
                    pos.sum(dim=1), neg.sum(dim=1), recons=False
                )
            ...
            if normalize:
                out = (
                    ...
                    * gt_resampled.abs().max(dim=-1, keepdim=True)[0]
                )
            ...
            mse_l = torch.nn.functional.mse_loss(
                out, gt_resampled, reduction="none"
            ).mean(dim=-1)  # weighted loss for each loss term
            ...
            imp_si_sdr_l = si_sdr_l - si_sdr_loss(
                audio.sum(dim=1), gt_resampled, zero_mean=True, load_diag=False
            )
    ...
    print(
        "average imp snr: ", np.array(acc_imp_snr).mean(), np.array(acc_imp_snr).std()
    )
    print("average si snr: ", np.array(acc_si_snr).mean(), np.array(acc_si_snr).std())
    print(
        "average imp si snr: ",
        np.array(acc_imp_si_snr).mean(),
        np.array(acc_imp_si_snr).std(),
    )
    print("average si sdr: ", np.array(acc_si_sdr).mean(), np.array(acc_si_sdr).std())
    print(
        "average imp si sdr: ",
        np.array(acc_imp_si_sdr).mean(),
        np.array(acc_imp_si_sdr).std(),
    )
...
print(
    f"evaling {' '.join([k + ':' + str(args[k]) for k in args])}, pos_num: {pos_num}, pos range: {partial_range}, neg range: {neg_partial_range}, hybrid: {hybrid_num}, neglect: {neglect_require_num}"
)
print(
    f"{test_dataloader.__class__.__name__} {source_num}C{''.join([str(n) for n in active_num])}, enroll {enroll_num}, wave length {wave_length}, pos {pos_example_length}, neg {neg_example_length}"
)
...
for epoch in range(repeat_num):
    (
        ...
        stoi_i,
        dnsmos_i,
    ) = eval(model=model, dataloader=test_dataloader, epoch=epoch)
    mses.append(mse_i)
    snrs.append(snr_i)
