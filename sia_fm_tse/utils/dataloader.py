...
class CIPIC_simulator:
    def __init__(self, brir_dir, sample_rate):
        ...
        self.CIPIC_rir_scenes = [
            sofar.read_sofa(sid, verbose=False) for sid in CIPIC_rir_scenes
        ]
    ...
    def sample_one_rir(self, hrtf):
        m_idx = random.sample(range(hrtf.Data_IR.shape[0]), 1)[0]
        _sr = int(hrtf.Data_SamplingRate)
        rir = hrtf.Data_IR[m_idx].astype(np.float32)
    ...
    def zero_angle_rir(self, hrtf):
        _sr = int(hrtf.Data_SamplingRate)
        rir = hrtf.Data_IR[self.face_to_face_idx].astype(np.float32)
        rir = torchaudio.functional.resample(
            torch.from_numpy(rir), _sr, self.sample_rate
        )[..., : self.sample_rate]
        return rir
...
class RRBRIR_simulator:
    def __init__(self, brir_dir, sample_rate):
        ...
        self.RRBRIR_rir_scenes = [
            sofar.read_sofa(sid, verbose=False) for sid in RRBRIR_rir_scenes
        ]
