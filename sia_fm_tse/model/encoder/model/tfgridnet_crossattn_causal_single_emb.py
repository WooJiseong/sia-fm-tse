# Copied just for `state_dict` utility.
"""
...
from espnet2.enh.separator.tfgridnet_separator import (
    GridNetBlock as GridNetBlock_original,
)
...
class TFGridNet_origcrossattn_causal_single_emb(AbsSeparator):
    ...
    def __init__(
        ...
        self.deconv = nn.ConvTranspose2d(
            emb_dim, self.n_srcs * 2, ks, padding=(self.t_ksize - 1, 1)
        )
    ...
    def init_buffers(self, batch_size, device):
        conv_buf = torch.zeros(
            batch_size, self.n_imics * 2, self.t_ksize - 1, self.n_freqs, device=device
        )
        deconv_buf = torch.zeros(
            batch_size, self.emb_dim, self.t_ksize - 1, self.n_freqs, device=device
        )
        istft_buf = torch.zeros(
            batch_size,
            self.n_srcs,
            self.n_freqs * 2,
            self.istft_lookback,
    ...
    def forward(
        ...
        batch, deconv_buf, istft_buf = self.decode(
            deconv_buf, batch, istft_buf, n_batch, n_frames, n_freqs, mix_std_
        )
    ...
    def decode(
        self, deconv_buf, batch, istft_buf, n_batch, n_frames, n_freqs, mix_std_
    ):
        # t2 = time.time()
        ...
        batch = batch.view(
            [n_batch, self.n_srcs, 2, n_frames, n_freqs]
        )  # [B, n_srcs, 2, n_frames, n_freqs]

        batch = batch.transpose(3, 4)  # (B, n_srcs, 2, n_fft//2 + 1, T)

        # Concat real and imaginary parts
        batch = torch.cat(
            [batch[:, :, 0], batch[:, :, 1]], dim=2
        )  # (B, n_srcs, nfft + 2, T)
...
class GridNetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.inter_linear = nn.Linear(hidden_channels * (bidirectional + 1), emb_dim)
        ...
        if self.use_attn:
            ...
            self.add_module(
                ...
                nn.Sequential(
                    ...
                    Lambda(
                        lambda x: (
                            x.reshape(x.shape[0], x.shape[1], x.shape[2], n_head, E)
                            .permute(0, 3, 1, 2, 4)
                    ...
                    Lambda(
                        lambda x: (
                            ...
                            .reshape(
                                x.shape[0] * n_head,
                                x.shape[1],
                                x.shape[2] * (emb_dim // n_head),
                            )
                        )
                    ),
                    LayerNormalization4DCF_causal_gridnet(
                        (n_freqs, emb_dim // n_head), eps=eps
                    ),
                ),
            )
            self.add_module(
    ...
    def init_buffers(self, batch_size, device):
        K_buf = torch.zeros(
            (batch_size * self.n_head, self.local_atten_len - 1, self.E * self.n_freqs),
            device=device,
        )
        V_buf = torch.zeros(
            (
        ...
        return dict(K_buf=K_buf, V_buf=V_buf, c0=c0, h0=h0)
    ...
    def forward(self, x, init_state=None):
        ...
        if self.chunk_causal:
            ...
            if self.use_attn:
                ...
                batch = V.reshape(
                    B0, self.n_head, self.n_freqs, self.V_dim, T0
                )  # [B, H, Q, C, T]
                batch = batch.transpose(2, 3)  # [B, H, C, Q, T]
                batch = batch.reshape(
                    B0, self.n_head * self.V_dim, self.n_freqs, T0
                )  # [B, HC, Q, T]
                batch = batch.permute(0, 3, 2, 1)  # [B, T, Q, HC]

                batch = self["attn_concat_proj"](batch)  # [B, T, Q * C]
                batch = batch.reshape(
                    batch.shape[0], batch.shape[1], out.shape[2], -1
                )  # [B, T, Q, C])
        ...
        return (
            out,
            init_state,
        )  # , [t0 - t0_0, t1 - t0, t2 - t2_0, t3 - t2, t5 - t4, t7 - t6]
